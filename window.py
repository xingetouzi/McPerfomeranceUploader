...
def to_csv(q, obj, *args, **kwargs):
    q.put(obj.to_csv(*args, **kwargs))
...
def name_setter(func):
    @wraps(func)
    def wrapper(self, *args, file=None, **kwargs):
        symbol, *name = os.path.basename(file).replace("  ", " ").split(" ")[:-1]  # get symbol + strategy name
        name = " ".join(name)
        return func(self, *args, name=name, symbol=symbol, file=file, **kwargs)
...
def ema_filter(func):  # filter ema future symbol.
    @wraps(func)
    def wrapper(self, *args, name=None, symbol=None, **kwargs):
        if "000000" in symbol:
            return self._handle_ignore(name)
        else:
            return func(self, *args, name=name, symbol=symbol, **kwargs)
...
class FileTransformer(object):
    ...
    def _handler_ignore(self, name=None):
        logging.info("忽略对策略[%s]的转换" % name if name else "未知")
        return "IGNORE"
    ...
    def _run(self):
        ...
        for path in self._files:
            ...
            try:
                if path.endswith(".xls") or path.endswith(".xlsx"):
                    self._pool.apply_async(to_csv, args=(self._q, self), kwds=dict(file=path))
                elif path.endswith(".zip"):
                    with zipfile.ZipFile(path) as z:
                        for name in z.filelist:
                            content = z.read(name)
                            self._pool.apply_async(to_csv, args=(self._q, self),
                                                   kwds=dict(file=name.filename, content=content))
            except Exception as e:
                self._handle_error(e)
                continue
        self._pool.close()
        ...
        while not self._q.empty():
            ...
            if result == "ERROR":
                ...
                ignore += 1
        self._q.close()
        logging.info("转换文件结束，成功转换文件总数:%s,错误数:%s, 忽略数%s." % (count, error, ignore))
    ...
    def to_csv(self, name=None, symbol=None, file=None, content=None):
        try:
            ...
            logging.warning("策略[%s]转换失败" % name)
            self._handle_error(e)
            return "ERROR"
        logging.info("策略[%s]转换成功" % (name if name else "未知"))
        return "OK"
